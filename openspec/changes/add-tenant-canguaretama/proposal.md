...
O Ofício Único de Registros e Notas de Canguaretama / RN (CNS 09.519-0) entra na plataforma
como mais uma serventia. Registrar um cartório é preencher configuração, não escrever código:
o objetivo é deixar o site público e o painel no ar sob o domínio já registrado,
`cartoriocanguaretamarn.com.br`, no mesmo molde de Major Sales, Taipu, Tibau do Sul e Mipibu.

## What Changes

- Novo arquivo de configuração `src/core/tenant/tenants/canguaretama.ts`, com os dados do
  cartório (slug, hosts, CNS, atribuições, contatos, endereço, titular, DPO, tema, textos
  institucionais).
- Registro do tenant em `src/core/tenant/resolve.ts` (`TENANTS`), o que habilita o host
  `canguaretama.localhost` em desenvolvimento e `cartoriocanguaretamarn.com.br` em produção.
...
- `public-site-foundation`: acrescenta o requisito de que a serventia de Canguaretama é servida
  pelos seus hosts próprios, com as seis atribuições e endereço cadastrado. Mesmo formato dos
  requisitos adicionados por Major Sales e por Taipu / Bento Fernandes; nenhum requisito
  existente muda.

## Impact

- `src/core/tenant/tenants/canguaretama.ts` (novo)
...
- Fora do código, a cargo do super admin: apontar o DNS de `cartoriocanguaretamarn.com.br` e
