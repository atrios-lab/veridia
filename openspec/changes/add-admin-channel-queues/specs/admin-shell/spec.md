...
#### Scenario: Os três canais do cidadão passam a existir na navegação

- **WHEN** um usuário com a permissão `channels.manage` abre a sidebar
- **THEN** os itens "Requerimentos LGPD", "Ouvidoria" e "Agenda de atendimentos" aparecem no grupo
  "Operação", cada um com o contador de itens em aberto daquele canal

#### Scenario: "Visão geral" é um destino de navegação com conteúdo

- **WHEN** qualquer usuário autenticado do painel abre a sidebar
- **THEN** o item "Visão geral" aponta para `/admin`, que mostra os contadores por canal, a
  atividade recente e os prazos a acompanhar em vez de um texto de placeholder
