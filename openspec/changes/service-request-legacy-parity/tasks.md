...
- [~] 6.2 Browser: **pendente, bloqueada.** `pnpm build` compila e o gate de sessão do painel redireciona certo. O fluxo renderizado não roda ainda: a migração 0011 não foi aplicada, e as credenciais do `.env` local estão rotacionadas (`password authentication failed`), então não consigo nem confirmar o estado do banco nem entrar no painel. Depende de `pnpm db:migrate` + sessão.
