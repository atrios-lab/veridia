...
do pedido for um e-mail: exigência registrada, pedido concluído, pedido cancelado, pedido
disponível para retirada, comprovante de pagamento recusado (retorno de "Pagamento informado"
para "Aguardando pagamento"), documento de entrega disponível, valor do pedido informado pela
primeira vez e formulário anexado a uma exigência. O aviso NÃO SHALL carregar o conteúdo (texto
da exigência, arquivo, valor, motivo da recusa): apenas o protocolo e a instrução de consultar
com a chave. O envio SHALL ser fire-and-forget: falha de e-mail nunca falha a ação.
...
#### Scenario: Disponível para retirada avisa
- **WHEN** o operador muda o andamento para "Disponível para retirada"
- **THEN** chega um aviso "o seu pedido está disponível para retirada" ao contato

#### Scenario: Comprovante recusado avisa
- **WHEN** o operador troca o andamento de um pedido em "Pagamento informado" de volta para
  "Aguardando pagamento"
- **THEN** chega um aviso de que o comprovante enviado não foi aceito e é preciso pagar de novo,
  sem o motivo da recusa

#### Scenario: Valor informado pela primeira vez não é confundido com comprovante recusado
- **WHEN** o operador informa o valor num pedido novo, gravando-o pela primeira vez em
  "Aguardando pagamento"
- **THEN** chega o aviso já existente de "valor informado", e não o aviso de comprovante
  recusado — a distinção é o andamento anterior do pedido, não o andamento novo
