...
- ~~`vercelAdapter` precisa mesmo de um `decide: () => false` manual para dev/CI sem `FLAGS`?~~ **Resolvido**, lendo o código-fonte de `flags/next` e `@vercel/flags-core`: `flag()` já envolve `decide()`/`adapter()` num try/catch e cai em `defaultValue` para qualquer erro, síncrono ou assíncrono — isso nunca dependeu do adapter, o `decide: () => false` manual do Edge Config era só uma otimização, não um requisito. Sem `FLAGS`, `vercelAdapter` tenta resolver um token OIDC (`getVercelOidcToken`) antes de qualquer fetch de rede; fora de um ambiente Vercel isso falha rápido e local (lê variável de ambiente, não faz chamada de rede), então o fallback para `defaultValue: false` acontece sem latência extra em dev/CI. Não é preciso reintroduzir a condicional — `adapter: vercelAdapter` incondicional (como já está em `src/flags.ts`) é suficiente.
- A SDK Key de Preview/Production é injetada automaticamente pela Vercel ou precisa ser declarada manualmente em Environment Variables? Ainda em aberto — depende do dashboard (task 1.3), não do código.
