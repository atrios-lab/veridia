...
escuro (`logos.seal.dark`), o nome da serventia e o rótulo "Painel administrativo". As cores,
tipografia e espaçamentos da casca SHALL vir dos tokens `--color-admin-*`, que são fixos da
plataforma e não variam por tenant.
...
#### Scenario: Tema do tenant não pinta o painel

- **WHEN** a serventia resolvida tem tema diferente de `verde-dourado`
- **THEN** a casca do painel continua com as mesmas cores, porque nenhuma variável
  `--palette-<tema>-*` é lida dentro de `/admin`
