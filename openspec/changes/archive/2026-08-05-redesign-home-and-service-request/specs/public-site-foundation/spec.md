...
O painel admin NÃO DEVE (SHALL NOT) herdar o tema do tenant: a estética da plataforma permanece fixa.

#### Scenario: Admin não tematizado
- **WHEN** um operador abre qualquer rota `/admin` em um host de tenant
- **THEN** a página não usa os tokens de marca do tenant
