...
| `admin-visual-identity.spec.ts` | — | Publicar/descartar tema é o mesmo CRUD de `tenant_content` acima; seção opcional some da navegação e seção obrigatória sem controle são `isSectionEnabled`/`SECTION_ROUTES`, já `tenant.test.ts`; abas e sessão, como acima. Conflito de merge com `main` (change `aumentar-limite-do-logotipo`, depois desta): os dois testes que chegaram para o limite de 3 MB do logotipo (recusa acima do limite, aceite até ele, com a mensagem exata) já são `src/core/tenant/brand-image.test.ts` ("a logo up to 3 MB passes", "a logo over 3 MB is rejected"), então a exclusão deste spec continua correta. |
